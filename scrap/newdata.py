...
kapiva_links_df = pd.read_excel('/home/vedant/ScrapData/scrap/ForestEssentailProductLink.xlsx')
# kapiva_links_df = pd.read_excel('/home/vedant/ScrapData/scrap/test.xlsx')
...
# Define a function to scrape product data
def scrape_product_data(link):
    try:
        ...
        try:
            ...
            print(f"Error fetching URL {link} with proxy {proxy}: {e}")
        ...
        # Scrape the entire product description HTML content
        product_description_div = soup.find('div', class_='ProductDescription__description-content___A_qCZ')
        if product_description_div:
            product_description_html = product_description_div.decode_contents()
        else:
            product_description_html = "Product description not found"

        # Scrape the product highlights
        product_highlight_div = soup.find('div', class_='ProductHighlights__highlights-text___dc-WQ')
        if product_highlight_div:
            product_highlights = product_highlight_div.decode_contents()
        else:
            product_highlights = "No product highlight section found"

        # Scrape manufacturer name and address
        ...
        if compliance_info_div:
            ...
            manufacturer_name = "Manufacturer info not found"
            address = "Address info not found"

        # Scrape the price (numeric only)
        price_span = soup.find('span', class_='PriceBoxPlanOption__offer-price___3v9x8')
        if price_span:
            price_text = price_span.get_text(strip=True).replace('₹', '').strip()
            price = ''.join(filter(str.isdigit, price_text))  # Keep only the numeric part
        else:
            price = "Price not found"

        # Scrape pack size and MRP (both selected and unselected packages)
        pack_size_mrp_list = []

        # Find all variants, including selected and unselected
        ...
        # Function to extract size and MRP from a div or a tag
        ...
        # Extract data for each variant in div (selected variants)
        for div in variant_divs:
            pack_size_mrp_list.append(extract_size_and_mrp(div))

        # Extract data for each variant in a (unselected variants)
        ...
        return "Error scraping data", "Error", "Error", "Price not found", [], "Images not found"

# Output file to store scraped data
output_file = "ForestEssentail.xlsx"

# Update the code that processes the output to include the new 'Product Description HTML' and 'Images' fields
if os.path.exists(output_file):
    ...
    for index, row in kapiva_links_df.iterrows():
        ...
        if 'Product Description HTML' in existing_data.columns:
            existing_data.at[index, 'Product Description HTML'] = product_description
        else:
            existing_data['Product Description HTML'] = pd.Series([product_description])

        if 'Product Highlights' in existing_data.columns:
            existing_data.at[index, 'Product Highlights'] = product_highlight
        else:
            existing_data['Product Highlights'] = pd.Series([product_highlight])

        if 'Manufacturer Name' in existing_data.columns:
            existing_data.at[index, 'Manufacturer Name'] = manufacturer_name
        else:
            existing_data['Manufacturer Name'] = pd.Series([manufacturer_name])

        if 'Address' in existing_data.columns:
            existing_data.at[index, 'Address'] = address
        else:
            existing_data['Address'] = pd.Series([address])

        if 'Price' in existing_data.columns:
            existing_data.at[index, 'Price'] = price
        else:
            existing_data['Price'] = pd.Series([price])

        if 'Images' in existing_data.columns:
            existing_data.at[index, 'Images'] = product_images
        else:
            existing_data['Images'] = pd.Series([product_images])

        existing_data.at[index, 'Pack Size MRP'] = json.dumps(pack_size_mrp_list, indent=4)

else:
    # Add new 'Images' field along with others
    kapiva_links_df[['Product Description HTML', 'Product Highlights', 'Manufacturer Name', 'Address', 'Price', 'Pack Size MRP', 'Images']] = kapiva_links_df['Link'].apply(lambda link: pd.Series(scrape_product_data(link)))
